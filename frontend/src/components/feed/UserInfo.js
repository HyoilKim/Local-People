...
export class UserInfo extends Component {
  state = {
    ...
    users: [],
    FeedsNameRef: db.collection("feeds").orderBy("timestamp", "desc").get(),
    names: [],
  ...
  renderUsers = (users, timestamp) =>
    users.map((user) => (
      <li>
        {" "}
        {user.location} {Math.floor((timestamp - user.creationTime) / 86400000)}
        일동안 거주 중
      </li>
    ));
  //serverTimestamp
  ...
  render() {
    ...
    return <div>{this.renderUsers(users, timestamp)}</div>;
  }
}
